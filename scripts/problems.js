...
xmlImporter.openTextFile("../quals/"+qualType+"/problemTypes.txt", null, function(line) {
    ...
    for (let type in problemTypes) loadProblemType(type);
});

function loadProblemType(problemType) {
    if (!(problemType in problemTypes)) console.warn(problemType + " is not in problemTypes.xml");
    xmlImporter.openTextFile("../quals/"+qualType+"/"+problemType+"List.txt", null, function(line) {
        console.log(line);
    });
}
